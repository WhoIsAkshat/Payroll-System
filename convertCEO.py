...
mymonth = str(previous_month.split("-")[-1])
ddate = mydate.strftime("%d-%m-%Y")
tdays = days_in_month(prev_month)
file_name = "Salary_"+str(last_day_of_previous_month.strftime('%B-%Y'))+"_CEO1.xlsx"
...
ws.cell(row=2, column=11, value='Days')
ws.cell(row=2, column=12, value=f"{tdays}")
...
ws.cell(row=2, column=16, value="Chq.")
ws.cell(row=2, column=17, value="Cash")
ws.cell(row=2, column=18, value="Days")
ws.cell(row=2, column=19, value="Hrs.")
ws.cell(row=2, column=20, value="T")
...
ws.cell(row=2, column=18).alignment = Alignment(horizontal='center')
ws.cell(row=2, column=19).alignment = Alignment(horizontal='center')
ws.cell(row=2, column=20).alignment = Alignment(horizontal='center')

add_thick_outside_borders(ws, 1, 1, 2, 20)
...
for item in data_rows:
    try:
        ...
        cursor.execute(query)
        att = cursor.fetchone()
        query = f"SELECT dept FROM emp WHERE emp_id={item[0]}"
        cursor.execute(query)
    ...
    ws.cell(row=row, column=2, value=str(item[1]))
    ws.cell(row=row, column=2).alignment = Alignment(horizontal='left')
    ws.cell(row=row, column=2).font = Font(bold=True)
    ws.cell(row=row, column=3, value=rem_advance - taken_adv + item[6])
    ...
    ws.cell(row=row, column=15).alignment = Alignment(horizontal='left')
    if item[12] == 'Y':
        ws.cell(row=row, column=15, value=f"Bonus: 200")
    else:
        ws.cell(row=row, column=15, value=f"Bonus: 0")

    ws.cell(row=row, column=18, value=round(att[2]*tdays)/100)
    ws.cell(row=row, column=19, value=att[3])
    ws.cell(row=row, column=20, value=int(att[4])*8)

    ws.cell(row=row+1, column=2, value=item[2])
    ws.cell(row=row+1, column=2).font = Font(bold=True)
    ws.cell(row=row+1, column=3, value=round(item[11]))
    ws.cell(row=row+1, column=4, value="+")
    ws.cell(row=row+1, column=5, value=round(((item[5]/8)/tdays)*item[2]))
    ws.cell(row=row+1, column=6, value="+")
    ws.cell(row=row+1, column=7, value=item[7])
    ws.cell(row=row+1, column=8, value="=")
    ws.cell(row=row+1, column=9, value=round((((item[5]/8)/tdays)*item[2])+item[7]+item[11]))
    ws.cell(row=row+1, column=10, value="-")
    ws.cell(row=row+1, column=11, value=item[3]+item[4])
    ws.cell(row=row+1, column=12, value="-")
    ws.cell(row=row+1, column=13, value=item[6])
    # if item[12] == 'Y':
    #     ws.cell(row=row+1, column=13, value=item[10]+item[3]+item[4]-200)
    # else:
    #     ws.cell(row=row+1, column=13, value=item[10]+item[3]+item[4])
    ws.cell(row=row+1, column=14, value="=")
    ws.cell(row=row+1, column=15, value=item[10])
    ws.cell(row=row+1, column=16, value=item[13])
    ws.cell(row=row+1, column=17, value=item[14])
    ws.cell(row=row+1, column=15).font = Font(bold=True)
    ws.cell(row=row+1, column=9).font = Font(bold=True)

    add_thick_outside_borders(ws, row, 1, row+1, 20)
...
def title_range(filepath):
    ...
    ws.cell(row=1, column=1, value=f"Salary Sheet:   {ss}")
